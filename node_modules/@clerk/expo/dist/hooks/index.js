"use strict";
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __reExport = (target, mod, secondTarget) => (__copyProps(target, mod, "default"), secondTarget && __copyProps(secondTarget, mod, "default"));
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
var hooks_exports = {};
__export(hooks_exports, {
  useAPIKeys: () => import_react.useAPIKeys,
  useClerk: () => import_react.useClerk,
  useEmailLink: () => import_react.useEmailLink,
  useOrganization: () => import_react.useOrganization,
  useOrganizationList: () => import_react.useOrganizationList,
  useReverification: () => import_react.useReverification,
  useSession: () => import_react.useSession,
  useSessionList: () => import_react.useSessionList,
  useSignIn: () => import_react.useSignIn,
  useSignUp: () => import_react.useSignUp,
  useUser: () => import_react.useUser,
  useWaitlist: () => import_react.useWaitlist
});
module.exports = __toCommonJS(hooks_exports);
var import_react = require("@clerk/react");
__reExport(hooks_exports, require("./useSSO"), module.exports);
__reExport(hooks_exports, require("./useOAuth"), module.exports);
__reExport(hooks_exports, require("./useAuth"), module.exports);
__reExport(hooks_exports, require("./useNativeSession"), module.exports);
__reExport(hooks_exports, require("./useNativeAuthEvents"), module.exports);
__reExport(hooks_exports, require("./useUserProfileModal"), module.exports);
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  useAPIKeys,
  useClerk,
  useEmailLink,
  useOrganization,
  useOrganizationList,
  useReverification,
  useSession,
  useSessionList,
  useSignIn,
  useSignUp,
  useUser,
  useWaitlist,
  ...require("./useSSO"),
  ...require("./useOAuth"),
  ...require("./useAuth"),
  ...require("./useNativeSession"),
  ...require("./useNativeAuthEvents"),
  ...require("./useUserProfileModal")
});
//# sourceMappingURL=index.js.map